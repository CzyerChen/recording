### mysql msi 安装
### mysql zip 包安装
- 64位安装包：下载地址：https://dev.mysql.com/downloads/mysql/
- 解压软件，配置MYSQL_HOME的环境变量，并添加path bin
- 生成data文件夹：mysqld.exe --initialize-insecure --user=mysql 
- net start mysql  启动mysql服务
- 如果启动失败
- 添加一个my.ini的文件，作为MySQL的配置文件
```text
...
log-output=FILE
general-log=0
general_log_file="LILY-PC.log"
slow-query-log=1
slow_query_log_file="LILY-PC-slow.log"
long_query_time=10
...
- 在进行：net start mysql 
- 启动成功之后
- mysql -u root -p  登录成功，则配置成功
- 因为root用户没有设置密码，因而需要初始化密码
```text
用下来，是方法二比较友好
